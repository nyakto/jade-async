...
DefaultPromiseProvider.prototype.isPromise = function (value) {
    ...
    if (prefix === this.name + '.') {
        return /^[^.]+\.(promiseArray|promiseObject|resolve|promise)\(.*\)$/.test(value);
    }
    return false;
};
...
AsyncCompiler.prototype.compile = function () {
    ...
    if (this.streamOnly) {
        fn += 'var write = jade.createWriter(buff, function (data) { stream.write(String(data)); });';
    } else {
        fn += 'var buff = [];';
    ...
    if (this.streamOnly) {
        fn += 'return write(' + this.mainBlock.name + '(), 0);';
    } else {
