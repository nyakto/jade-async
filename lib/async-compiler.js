...
function Block(compiler, name, params) {
    this.simple = true;
    this.header = false;
...
AsyncCompiler.prototype.createBlock = function (params) {
    var block = new Block(this, '$b' + this.blocks.length, params || []);
...
AsyncCompiler.prototype.visitCase = function (node, block) {
    var body = this.createBlock(['$caseValue']);
...
AsyncCompiler.prototype.visitEach = function (node, block) {
    var itemBlock = this.createBlock([node.val, node.key]);
    this.visit(node.block, itemBlock);
    var alternativeBlock = null;
    if (node.alternative) {
        alternativeBlock = this.createBlock();
